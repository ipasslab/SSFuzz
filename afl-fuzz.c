...
{
  int trans_hash; // from_state_id ^ (to_state_id<<1)
  unsigned int from_state_id;
  unsigned int to_state_id;
  unsigned int msg_pool_size;
  unsigned int *msg_pool; // msg idx in interesting_msg_pool_size
};
unsigned int transition_pool_size = 0;
struct state_transition *transition_pool;
struct state
{
  unsigned int state_id;

  unsigned int next_state_count;
  int *next_states; /*state ids*/
};
unsigned int state_pool_size = 0;
struct state *state_pool;

unsigned int interesting_msg_pool_size = 0;
struct M_oneMessage **interesting_msg_pool;

struct M_oneMessage *msg_cur = NULL;

unsigned int msg_sent_max = 30;
/*during send_over_network_MY, virgin_bits change. this help recover.*/
EXP_ST u8 SR_virgin_bits[MAP_SIZE];

boolean debug_info_record = FALSE;
FILE *debug_info_file = NULL;

boolean test_mode = TRUE;
boolean trans_mode = FALSE;
boolean dispatch_mode = TRUE;

kliter_t(lms) *M2_prefix_last_msg = NULL;
kliter_t(lms) *M3_first_msg = NULL;

/*MY function*/
static u32 choose_block_len(u32 limit); /*delare here for havoc_mutation.*/
int find_trans(int hash);
int get_state_idx(unsigned int state_id);
/*add interesting msg to pool, meanwhile add msg to transition pool*/
void add_msg_to_pool(struct M_oneMessage *Message, unsigned int state_id_from, unsigned int state_id_to)
{
  /*add msg to interesting msg pool*/
  interesting_msg_pool_size++;
  interesting_msg_pool = realloc(interesting_msg_pool, interesting_msg_pool_size * sizeof(struct M_oneMessage *));
  unsigned int msg_idx = interesting_msg_pool_size - 1;
  interesting_msg_pool[msg_idx] = Message;

  /*add msg to transition pool*/
  int hash = state_id_from ^ (state_id_to << 1);
  int trans_idx = find_trans(hash);
  int state_idx;
  ...
  /*new transition*/
  {
    ...
    transition_pool = realloc(transition_pool, transition_pool_size * sizeof(struct state_transition));
    transition_pool[transition_pool_size - 1].trans_hash = hash;
    transition_pool[transition_pool_size - 1].from_state_id = state_id_from;
    transition_pool[transition_pool_size - 1].to_state_id = state_id_to;
    transition_pool[transition_pool_size - 1].msg_pool_size = 1;
    transition_pool[transition_pool_size - 1].msg_pool = malloc(1 * sizeof(int));
    transition_pool[transition_pool_size - 1].msg_pool[0] = msg_idx;

    /*add new next_state to cur_state*/
    state_idx = get_state_idx(state_id_from);
    if (state_idx == -1)
      FATAL("state pool error, state pool size %d, state_id_from %u.", state_pool_size, state_id_from);
    state_pool[state_idx].next_state_count += 1;
    state_pool[state_idx].next_states = realloc(state_pool[state_idx].next_states, state_pool[state_idx].next_state_count * sizeof(int));
    state_pool[state_idx].next_states[state_pool[state_idx].next_state_count - 1] = state_id_to;
  }
  else
  /*add new interesting msg to old trans*/
  {
    transition_pool[trans_idx].msg_pool_size += 1;
    transition_pool[trans_idx].msg_pool = realloc(transition_pool[trans_idx].msg_pool, transition_pool[trans_idx].msg_pool_size * sizeof(int));
    transition_pool[trans_idx].msg_pool[transition_pool[trans_idx].msg_pool_size - 1] = msg_idx;
  }
  /*add new state to state pool*/
  state_idx = get_state_idx(state_id_to);
  if (state_idx == -1)
  /*new state*/
  {
    state_pool_size++;
    state_pool = realloc(state_pool, state_pool_size * sizeof(struct state));
    state_pool[state_pool_size - 1].state_id = state_id_to;
    state_pool[state_pool_size - 1].next_state_count = 0;
    state_pool[state_pool_size - 1].next_states = NULL;
  }
  ...
  //   state_pool_size++;
  //   state_pool = realloc(state_pool, state_pool_size * sizeof(struct state));
  //   state_pool[state_pool_size - 1].state_id = state_id_from;
  //   state_pool[state_pool_size - 1].next_state_count = 0;
  //   state_pool[state_pool_size - 1].next_states = NULL;
  // }
  return;
}
/*get state_idx in pool by state_id*/
int get_state_idx(unsigned int state_id)
{
  for (int i = 0; i < state_pool_size; i++)
  {
    if (state_pool[i].state_id == state_id)
      return i;
  }
  return -1;
}

/*get trans_idx in pool by trans_hash*/
int find_trans(int hash)
{
  for (int i = 0; i < transition_pool_size; i++)
  {
    if (transition_pool[i].trans_hash == hash)
      return i;
  }
  return -1;
}
void havoc_mutation(struct M_oneMessage *Message)
{
  // if (dispatch_mode == trans_mode)
  // {
  //   dispatch_mode = test_mode;
  //   return;
  // }
  // else
  // {
  //   dispatch_mode = trans_mode;
  // }
  s32 temp_len, use_stacking;
  u8 *out_buf = NULL;
  /*********************************************
...
#define FLIP_BIT(_ar, _b)                     \
  ...
  temp_len = Message->size;
  out_buf = ck_realloc(out_buf, temp_len);
  memcpy(out_buf, Message->content, temp_len);
  ...
  if (temp_len != Message->size)
  {
    Message->content = realloc(Message->content, temp_len);
    Message->size = temp_len;
  }
  memcpy(Message->content, out_buf, temp_len);
  ck_free(out_buf);
#undef FLIP_BIT
}
/*0.add an interesting msg.
  1.add a new transition.
  2.add a new interesting msg in an old transition.*/
void record_interesting_sth(unsigned int pre_state_id, unsigned int state_id, struct M_oneMessage *cur_message, u8 hnb)
{
  int trans_hash = (pre_state_id) ^ (state_id << 1);
  int idx = find_trans(trans_hash);
  // unsigned int msg_idx;
  /*add an interesting msg, interesting st. new cov or new trans*/
  if (hnb == 2 /*new cov*/ || idx == -1 /*new transition*/)
  {

    add_msg_to_pool(cur_message, pre_state_id, state_id);
    // msg_idx = interesting_msg_pool_size - 1;
  }
  else // free cur_message
  {
    free(cur_message->content);
    free(cur_message);
    cur_message = NULL;
  }
  return;
}
/*dispatch a cpoy of message.*/
struct M_oneMessage *dispatch_message(unsigned int state_id)
{
  unsigned int msg_idx;
  if (dispatch_mode == trans_mode)
  {
    /*get a rand next state*/
    int state_idx = get_state_idx(state_id);
    int next_states_count = state_pool[state_idx].next_state_count;
    if (next_states_count == 0) /*no after state, random dispatch*/
    {
      msg_idx = UR(interesting_msg_pool_size);
    }
    else
    {
      int next_state_idx = UR(next_states_count);
      int next_state_id = state_pool[state_idx].next_states[next_state_idx];
      int trans_hash = (state_id) ^ (next_state_id << 1);
      /*get a rand msg of this trans*/
      int trans_idx = find_trans(trans_hash);
      if (trans_idx == -1)
        FATAL("error get a non-exsiting trans, from state_id %d to state_id %d.\n", state_id, next_state_id);
      int trans_msg_idx = UR(transition_pool[trans_idx].msg_pool_size);
      msg_idx = transition_pool[trans_idx].msg_pool[trans_msg_idx];
    }
  }
  else /*test mode*/
  {
    ...
    int state_idx = get_state_idx(state_id);
    if (state_idx == -1)
      FATAL("error get a non-exsiting state %u ,target_state_id %u.", state_id, target_state_id);
    int next_states_count = state_pool[state_idx].next_state_count;
    if (next_states_count == 0) /*no after state, random dispatch*/
    {
      msg_idx = UR(interesting_msg_pool_size);
  ...
  struct M_oneMessage *msg_copy = (struct M_oneMessage *)malloc(sizeof(struct M_oneMessage));
  // int idx = UR(interesting_msg_pool_size);
  msg_copy->size = interesting_msg_pool[msg_idx]->size;
  msg_copy->content = malloc(msg_copy->size);
  memcpy(msg_copy->content, interesting_msg_pool[msg_idx]->content, msg_copy->size);
  return msg_copy;
}
...
int send_over_network_MY()
{
  ...
  unsigned int state_count, state_id, pre_state_id = target_state_id;
  ...
  // write the request messages, send M1 & M2_prefix
  kliter_t(lms) * it;
  messages_sent = 0;
  if (debug_info_record)
  {
    fprintf(debug_info_file, "msg_seq:");
    for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it))
    {
      fprintf(debug_info_file, "%p->", it);
    }
    fprintf(debug_info_file, ",kl_end:%p,last_M2:%p,first_M3:%p,", kl_end(kl_messages), M2_prefix_last_msg, M3_first_msg);
  }

  for (it = kl_begin(kl_messages); it != kl_next(M2_prefix_last_msg); it = kl_next(it))
  {
    n = net_send(sockfd, timeout, kl_val(it)->mdata, kl_val(it)->msize);
    messages_sent++;
  ...
  if (state_count >= 2)
  {
    state_id = state_sequence[state_count - 1];
  }
  else
  { /*response is unexpected, state_count=1 or ?*/
    state_id = 9999;
  }
  ck_free(state_sequence);
  msg_cur = (struct M_oneMessage *)malloc(sizeof(struct M_oneMessage));
  msg_cur->size = kl_val(M2_prefix_last_msg)->msize;
  msg_cur->content = malloc(msg_cur->size);
  memcpy(msg_cur->content, kl_val(M2_prefix_last_msg)->mdata, msg_cur->size);
  record_interesting_sth(pre_state_id, state_id, msg_cur, hnb);

  /*send M2_postfix.*/
  message_t *m;
  dispatch_mode = test_mode;
  kliter_t(lms) * the_new_msg, *pre_M3_last_msg;
  if (M3_first_msg == kl_end(kl_messages))
    M3_first_msg = NULL;
  while (messages_sent < msg_sent_max)
  {
    /*construct kl_messages while send msg.*/
    msg_cur = dispatch_message(state_id);
    havoc_mutation(msg_cur);
    m = (message_t *)ck_alloc(sizeof(message_t));
    m->mdata = (char *)ck_alloc(msg_cur->size);
    m->msize = msg_cur->size;
    memcpy(m->mdata, msg_cur->content, m->msize);
    pre_M3_last_msg = get_last_message(kl_messages);
    *kl_pushp(lms, kl_messages) = m;
    if (debug_info_record)
    {
      the_new_msg = get_last_message(kl_messages);
      fprintf(debug_info_file, "push:%p,", the_new_msg);
    }
    the_new_msg = get_last_message(kl_messages);
    if (M3_first_msg != NULL)
    {
      /*mov the new msg to the palce between M2 and M3*/
      kl_next(M2_prefix_last_msg) = the_new_msg;
      kl_next(the_new_msg) = M3_first_msg;
      kl_next(pre_M3_last_msg) = kl_end(kl_messages);
    }
    M2_prefix_last_msg = the_new_msg;

    n = net_send(sockfd, timeout, msg_cur->content, msg_cur->size);
    messages_sent++;
    ...
    else
    {
      ...
      state_sequence = (*extract_response_codes)(response_buf + prev_buf_size, response_buf_size - prev_buf_size, &state_count);
      if (state_count >= 2)
      {
        state_id = state_sequence[state_count - 1];
      }
  ...
  if (debug_info_record)
  {
    fprintf(debug_info_file, "msg_seq2:");
    for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it))
    {
      fprintf(debug_info_file, "%p->", it);
    }
  ...
  /*send M3*/
  for (it = kl_next(M2_prefix_last_msg); it != kl_end(kl_messages); it = kl_next(it))
  {
    n = net_send(sockfd, timeout, kl_val(it)->mdata, kl_val(it)->msize);
    messages_sent++;
...
HANDLE_RESPONSES:
  ...
  if (messages_sent > kl_messages->size)
  {
    if (debug_info_record)
    {
      fprintf(debug_info_file, "error!");
      fflush(debug_info_file);
    }
    FATAL("error!messages_sent:%u > kl_messages_size:%lu", messages_sent, kl_messages->size);
  }
  if (debug_info_record)
  {
    fprintf(debug_info_file, "msg_sent:%u,", messages_sent);
    unsigned int state_count;
    unsigned int *state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);
    fprintf(debug_info_file, "state_seq:");
    for (int ii = 0; ii < state_count; ii++)
    {
      fprintf(debug_info_file, "%d->", state_sequence[ii]);
    }
    ck_free(state_sequence);
  }
...
static u8 run_target_MY(char **argv, u32 timeout)
{
  ...
  {
    // recover virgin
    memcpy(virgin_bits, SR_virgin_bits, MAP_SIZE);
    u8 hnb = has_new_bits(virgin_bits);
    fprintf(debug_info_file, "all_hnb:%u,", hnb);
  }
  prev_timed_out = child_timed_out;
...
static void perform_dry_run(char **argv)
{
  ...
  while (q)
  {
    ...
    /*MYfuzz code*/
    u32 i = 0;
    unsigned int pre_state, cur_state = 0;
    kliter_t(lms) * it;
    struct M_oneMessage *msg;
    ...
    if (debug_info_record)
    {
      ...
      for (i = 0; i < state_count; i++)
      {
        fprintf(debug_info_file, "%u->", state_sequence[i]);
      }
    ...
    for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it))
    {
      msg = (struct M_oneMessage *)malloc(sizeof(struct M_oneMessage));
      msg->size = it->data->msize;
      msg->content = malloc(msg->size * sizeof(char));
      memcpy(msg->content, it->data->mdata, it->data->msize);
      pre_state = cur_state;
      if ((response_bytes[i] == 0) || (i > 0 && (response_bytes[i] - response_bytes[i - 1] == 0)))
      {
        cur_state = 9999; // no-response state
      }
      else
      {
        state_sequence = (*extract_response_codes)(response_buf, response_bytes[i], &state_count);
        cur_state = state_sequence[state_count - 1];
        ck_free(state_sequence);
      }
      add_msg_to_pool(msg, pre_state, cur_state);
      i++;
    }
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault)
{
  ...
  if (debug_info_record)
  {
    fprintf(debug_info_file, "fault:%u,crash_mode:%u,", fault, crash_mode);
  }
  if (fault == crash_mode)
  {
    ...
    {
      if (hnb)
        fprintf(debug_info_file, "hnb:%u,", hnb);
      else
        fprintf(debug_info_file, "hnb:%u.", hnb);
    }
    if (!(hnb))
    ...
    if (debug_info_record)
    {
      fprintf(debug_info_file, "add_to_queue.");
    }
    if (state_aware_mode)
...
EXP_ST u8 common_fuzz_stuff_MY(char **argv, u8 *out_buf /*no use*/, u32 len /*no use*/)
{
  ...
  cur_last_message = get_last_message(kl_messages);
  M2_prefix_last_msg = cur_last_message;
  ...
  M3_first_msg = kl_next(M2_prefix_last_msg);
  /* End of AFLNet code */

  /*backup*/
  memcpy(SR_virgin_bits, virgin_bits, MAP_SIZE);

  fault = run_target_MY(argv, exec_tmout);
  ...
  /* This handles FAULT_ERROR for us: */
  u8 tmp = save_if_interesting(argv, out_buf, len, fault);
  queued_discovered += tmp;
...
havoc_stage:
  ...
  if (debug_info_record)
  {
    fprintf(debug_info_file, "fuzz_one:\n1st Target state id:%u,stage_max:%d,", target_state_id, stage_max);
  }
  if (debug_info_record)
  {
    if (M2_prev == NULL)
      fprintf(debug_info_file, "M1_msg_count:%d:\n", 0);
    else
    {
      int M1_msg_count = 0;
      kliter_t(lms) * it;
      for (it = kl_begin(kl_messages); it != kl_next(M2_prev); it = kl_next(it))
      {
        M1_msg_count += 1;
      }
      fprintf(debug_info_file, "M1_msg_count:%d:\n", M1_msg_count);
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++)
  {
    ...
    if (debug_info_record)
    {
      fprintf(debug_info_file, "stage_cur:%u,", stage_cur);
    }
    ...
    if (common_fuzz_stuff_MY(argv, out_buf, temp_len))
      goto abandon_entry;
    if (debug_info_record)
    {
      fprintf(debug_info_file, "msg_pool_size:%u,state_pool_size:%u,trans_pool_size:%u\n", interesting_msg_pool_size, state_pool_size, transition_pool_size);
    }
...
int main(int argc, char **argv)
{
  ...
  /*MYfuzz code for debug*/
  state_pool_size = 1;
  state_pool = malloc(state_pool_size * sizeof(struct state));
  state_pool[0].state_id = 0;
  state_pool[0].next_state_count = 0;
  state_pool[0].next_states = NULL;

  if (debug_info_record)
  {
    debug_info_file = fopen("./debug_info.txt", "w");
  }
  /*MYfuzz code end.*/
  struct timeval tv;
  struct timezone tz;
