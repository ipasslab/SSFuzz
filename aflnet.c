...
unsigned int *extract_response_codes_SNTP(unsigned char *buf, unsigned int buf_size, unsigned int *state_count_ref)
{
  ...
  while (byte_count < buf_size)
  {
    ...
    if ((mem_count > 0) && ((memcmp(&mem[mem_count - 1], terminator_one, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_two, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_three, 1) == 0)))
    {
      // Extract the response code which is the first 4 bytes
...
unsigned int *extract_response_codes_SNMP(unsigned char *buf, unsigned int buf_size, unsigned int *state_count_ref)
{
  ...
  while (byte_count < buf_size)
  {
    ...
    if ((mem_count > 7) && ((memcmp(&mem[mem_count - 1], terminator_one, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_two, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_three, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_four, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_five, 1) == 0) || (memcmp(&mem[mem_count - 1], terminator_six, 1) == 0)))
    {
      // Extract the response code which is the first 4 bytes
      char temp[2];
